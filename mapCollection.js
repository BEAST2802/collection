...
class mapCollection extends Map {
   ...
   * @returns Collection
   * @example new mapCollection({"hii": "hello", "ok": "bye"})
   * @example new mapCollection([["hii", "hello"], ["ok", "bye"]])
   */
  constructor(data) {
    if (!data) {
  ...
  /**
   * @param {fn: function(Value: v, Key: k, Collection: this)} - Function for running on the entries of the collection
   * @description Performs a forEach function on the entries of the collection
   * @returns Value returned from the function or undefined
   * @example Collection.each((v, k, collection) => { console.log(`Key: ${k}, Value: ${v}`)})
   */
  forEach(fn) {
  ...
  /**
   * @param {collection} - Map or instance of Map to compare with the current collection
   * @description Compares both Map or instance of Map and return true if they are equal
   * @returns boolean
   * @example Collection.equal(mapCollection)
   */
  equal(collection) {
  ...
  /**
   * @param {key: Any} - The key from the collection
  ...
  /**
   * @param {collections} [Map or instance of Map]
